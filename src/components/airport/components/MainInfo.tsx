...
export default function MainInfo({ icao }: { icao: string }) {
    ...
    const { stationsData } = useControllerStore()
    ...
    const openStationsInfo = (e: React.MouseEvent<HTMLElement>) => {
        setHeight(e, panelStates.stations)
        setPanelStates((prevState) => ({
            ...prevState,
            stations: !panelStates.stations
        }))
    }
        ...
        <>
            ...
            <div className="info-panel-container column main scrollable">
                ...
                {stationsData.length > 0 &&
                    <div className={`info-panel-container column sub dropdown ${panelStates.stations ? 'open' : ''}`}>
                        <button className='info-panel-container-header' onClick={openStationsInfo}>
                            <p>Terminal Information</p>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" className={`info-panel-header-dropdown ${panelStates.stations ? 'open' : ''}`}>
                                <path fillRule="evenodd" d="M11.842 18 .237 7.26a.686.686 0 0 1 0-1.038.8.8 0 0 1 1.105 0L11.842 16l10.816-9.704a.8.8 0 0 1 1.105 0 .686.686 0 0 1 0 1.037z" clipRule="evenodd"></path>
                            </svg>
                        </button>
                        <div className='info-panel-container sub'>
                            ...
                            <div className="info-panel-container-content" id='airport-panel-stations'>
                                {stationsData.map(station => <ActiveController key={station.callsign} station={station} />)}
                            </div>
                        </div>
                    </div>
                }
