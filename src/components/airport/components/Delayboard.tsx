export default function Delayboard({}) {
    ...
    let delayColor = "rgb(77, 95, 131)"
    const delay = flightsRef.current.dir === 'departures' ? airportDataRef.current.stats.departure?.tDelay : airportDataRef.current.stats.arrival?.tDelay

    if (delay <= 30) {
        delayColor = 'rgb(11, 211, 167)'
    }
    if (delay > 30) {
        delayColor = 'rgb(234, 89, 121)'
    }

    const dir = flightsRef.current.dir

    return (
        <div className="airport-delayboard">
            <div className="airport-delayboard-header">
                CURRENT DISRUPTIONS
            </div>
            <div className="airport-delayboard-main">
                <div className="airport-delayboard-element">
                    <div className="airport-delayboard-title">
                        AVERAGE DELAY
                    </div>
                    <div className="airport-delayboard-content" style={{ color: delayColor }}>
                        {delay}
                        <span>min</span>
                    </div>
                </div>
                <div className="airport-delayboard-element">
                    <figure className="airport-delayboard-icon">
                        {dir === 'departures' ?
                    ...
                    <div className="airport-delayboard-separator"></div>
                </div>
                <div className="airport-delayboard-element">
                    <div className="airport-delayboard-title">
                        FLIGHTS DELAYED
                    </div>
                    <div className="airport-delayboard-content">
                        {flightsRef.current.dir === 'departures' ? airportDataRef.current.stats.departure?.nDelay : airportDataRef.current.stats.arrival?.nDelay}
                    </div>
                </div>
            </div>
        </div>
